...
const Pagination = ({ currentPage, setCurrentPage, postsPerPage, onLoadMore, setSkip, setFilteredBlogs }: Props) => {
  const { data: paginationData, error } = useQuery<RecordsQuery>(blogsOps.Queries.getTotalRecords);
  ...
  function getCachedBlogs(skipBlogs: number) {
    const { blogs } = client.readQuery({
      query: blogsOps.Queries.getBlogs,
      variables: {
        skip: skipBlogs,
        first: postsPerPage,
      },
        ...
        {new Array(totalPages).fill(0).map((_, idx) => (
          <span
            ...
            className={`text-[1.4rem] ${
              currentPage === idx + 1 ? "text-main-orange" : "text-gray-500 hover:text-main-orange transition-all"
